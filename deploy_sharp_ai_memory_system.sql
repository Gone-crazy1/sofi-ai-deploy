-- DEPLOY COMPREHENSIVE MEMORY & AWARENESS SYSTEM
-- Execute this in Supabase SQL Editor for Sharp AI Memory
...
-- Create functions for automatic spending analytics
CREATE OR REPLACE FUNCTION update_spending_analytics()
RETURNS TRIGGER AS $$
BEGIN
    -- Auto-update user spending patterns when transaction is recorded
    ...
    VALUES (
        ...
        'daily',
        DATE_TRUNC('day', NEW.transaction_date),
        DATE_TRUNC('day', NEW.transaction_date) + INTERVAL '1 day',
        NEW.amount,
        1
    )
    ON CONFLICT (telegram_chat_id, period_start, period_end, period_type)
    DO UPDATE SET
        total_spent = spending_analytics.total_spent + NEW.amount,
        transaction_count = spending_analytics.transaction_count + 1;
...
CREATE TRIGGER transaction_analytics_trigger
    AFTER INSERT ON transaction_memory
    FOR EACH ROW
    EXECUTE FUNCTION update_spending_analytics();
